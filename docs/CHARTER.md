# GGZERO — 项目工作宪章（自研聚合 LLM 中转站）

> **代号** `ggzero`（产品显示名待你定；我会把显示名做成可改设置，符合"高自定义"愿景）。
...
**底座 = octopus（lingyu 版）**，本地工作区 `ggzero/`，从 `参考项目/octopus-master-lingyu版` 导入。
...
| 期 | 目标 | DoD |
|----|------|-----|
| **P0 构建基线** | 前端+后端全量 build 出二进制并本地跑起来 | `ggzero.exe` 启动、浏览器进首页、能登录/看控制台 ✅ 为后续前提 |
...
- 工作区 `ggzero/` 从 lingyu 版导入，git 初始化（`9332865`）。
