...
- **D2 一键商业开关**：octopus 商业面较薄（无内建注册/计费/支付），「释放商业潜力」需先补商业功能再做开关 —— 是独立的较大模块。
- **D3c 主题服务端按账户持久化**：当前每用户选择存浏览器 localStorage；做成绑账户跨设备需后端 user-pref 表。
- **冬日落地页视觉精修**：当前是主题 token 自适配的纯 CSS 版（无大图/无 color4bg 外部库）；如要承接原图（松树少女雪景）+ color4bg 氛围光，需加 public 资源（视觉件，建议你看过再调）。
- README 全量改写为 GGZERO（当前 NOTICE + docs 已是新身份）。
...
`9332865` 导入底座 → `6893eda` P0 跑通 → `d62357a` P1 改名 → `40afe15` P3a 主题引擎 → `5be6599` P3b 可上传主题 → `73b38d6` P4 冬日落地页。
（本地 git，未 push。）
