...
>
> **占位符约定**：文中 `YOUR_DB_HOST`、`YOUR_DB_USER`、`YOUR_SSH_HOST`、`YOUR_SSH_PORT`、
> `YOUR_SSH_USER`、`lodestar.example.com` 均为占位符，执行前替换成你自己环境的实际值。
> 真实主机地址与凭据只应存在于服务器上的 `.env`，不要写回本仓库。
...
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "DROP DATABASE IF EXISTS lodestar;"
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "CREATE DATABASE lodestar OWNER YOUR_DB_USER;"

# 3. 从 dump 恢复
pg_restore -h YOUR_DB_HOST -U YOUR_DB_USER -d lodestar \
  --clean --if-exists \
  data/backup/lodestar-YYYYMMDD_HHMMSS.dump

# 4. 验证
psql -h YOUR_DB_HOST -U YOUR_DB_USER -d lodestar -c "SELECT COUNT(*) FROM users;"
psql -h YOUR_DB_HOST -U YOUR_DB_USER -d lodestar -c "\dt"
...
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "DROP DATABASE IF EXISTS lodestar_log;"
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "CREATE DATABASE lodestar_log OWNER YOUR_DB_USER;"
```

### 6.3 网络中断

**症状**：无法访问 `lodestar.example.com`，但容器仍在运行
...
# 3. 如果连接数耗尽
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "SELECT count(*) FROM pg_stat_activity;"
# 清理空闲连接
psql -h YOUR_DB_HOST -U YOUR_DB_USER -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state = 'idle' AND query_start < now() - interval '10 minutes';"
...
```bash
# 1. 确认 Redis 状态
redis-cli -h YOUR_DB_HOST -n 2 ping
# 预期：PONG
...
| 服务 | 地址 | 备注 |
|------|------|------|
| Lodestar 前台 | https://lodestar.example.com | Cloudflare Tunnel |
| 服务器 SSH | YOUR_SSH_HOST:YOUR_SSH_PORT | 用户 YOUR_SSH_USER |
| 服务器部署目录 | /opt/docker/lodestar/ | .env + docker-compose.yml + data/ |
| PostgreSQL | YOUR_DB_HOST:5432 | 库 lodestar |
| Redis | YOUR_DB_HOST:6379/db=2 | 可选，回落内存缓存 |
