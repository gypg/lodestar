...
>
> 最后更新：2026-06-19（A 线身份改造完成 + B 线消费级增量 + 地基安全债 1/3）。
...
### 地基安全债（审查发现，优先于新功能 —— 完成 1/3）
1. ✅ **维护模式后端守卫**（6d88ed0）
2. ⬜ **op 层 API Key 隔离脆弱点**：`internal/op/apikey/apikey.go` 的 `List` **不按 UserID 过滤**，多租户隔离全靠 handler 层 `listAPIKey` 的 filter。任何新 handler 复用 `op.List` 忘了 filter 就会跨租户泄露 key。建议加 `ListByUser(uid)` 或在 op 层加可选 user 过滤，让隔离不依赖每个 handler 的自觉。**注意**：admin/staff 要能看全部，别破坏 staff 视图（见 handlers/apikey.go 的 isStaff 分支）。
...
- `go test ./...` 全绿（0 FAIL）。
- 地基安全债 1/3 完成（维护守卫），下一个接手从 §6 的 #2（op 层 key 隔离）继续。
