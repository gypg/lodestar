...
>
> 最后更新：2026-06-19（地基安全债 **3/3 完成**；本地可能 ahead 未 push，见 §9）。
...
### 地基安全债（审查发现，优先于新功能 —— **完成 3/3**）
1. ✅ **维护模式后端守卫**（6d88ed0）
2. ✅ **op 层 API Key 隔离**（af39c40）：`apikey.ListByUser(uid)` + `listAPIKey`/`getUsage`；测试 `internal/op/apikey/apikey_test.go`。
3. ✅ **余额事后扣费防负余额**（待提交 commit）：`DeductQuota` 改为 `quota - MIN(quota, amount)` / `used_quota + MIN(quota, amount)` 单条原子 UPDATE；并发多请求最多扣到 0，**不会负余额**（可能少扣费，未做预授权冻结）。测试 `internal/op/user/quota_test.go`。未改 relay 热路径预检逻辑（仍 `HasBalanceForKey` > 0）。

### 功能增量（B 线剩余 — 见 **`docs/HARVEST-REVIEW-2026-06.md`** 复审优先级）
- **P0 建议**：BaseURL 时延自测（SAPI）、用量曲线/热力图、Image 模型下拉、push ahead 提交。
- 其余：Ops 健康看板 UX、color4bg 可选、Chat 持久化、billingexpr/OAuth/订阅（按需）。
...
- **计费**：`op/billing.ChargeKey` → `user.DeductQuota`（**MIN 封顶**，不负余额）+ `middleware/auth.go` HasBalanceForKey（commercial_mode 开且余额>0）。事后扣费；若要严格按请求预扣需另做冻结表（未做）。
...
- 本地 commit 线（最近）：`fix(quota)`（#3，若已提交）→ `af39c40`（key 隔离）→ `d1806ea`（HANDOFF）→ `6d88ed0`（维护守卫）。**若 `git status` 显示 ahead**：含未推远端提交，执行 `git push origin main`（此前 HTTPS 曾 timeout）。
- `go test ./...` 全绿。
- **地基安全债 3/3 完成**。下一优先：**§6 功能增量**（测速/健康看板/Chat 持久化等）或 **收尾债**。
- 详细「多轮中断后的工作记录」见 **`docs/SESSION-LOG.md`**。
