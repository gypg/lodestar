...
<img src="web/public/logo.svg" alt="Lodestar Logo" width="120" height="120">
...
services:
  lodestar:
    ...
    volumes:
      - ./data:/app/data
    environment:
      LODESTAR_AUTH_JWT_SECRET: "replace-with-a-long-random-secret"
```
...
cd lodestar
# 可选：通过环境变量预置初始管理员账户
export LODESTAR_INITIAL_ADMIN_USERNAME="admin"
...
- 设置 `LODESTAR_INITIAL_ADMIN_USERNAME` 和 `LODESTAR_INITIAL_ADMIN_PASSWORD`，在启动时自动创建初始管理员账户
- 或在首次访问 Web UI 时，在引导向导页面中手动创建初始管理员账户

> ⚠️ **安全提示**：初始管理员密码长度必须至少为 12 个字符。
>
> ⚠️ **安全提示**：如果未配置 `LODESTAR_AUTH_JWT_SECRET` 或 `auth.jwt_secret`，Lodestar 会在启动时生成仅当前进程有效的 JWT 密钥。服务重启后，已有登录 token 会失效。
...
> 💡 **提示**：在生产环境运行 Lodestar 前，请设置 `LODESTAR_AUTH_JWT_SECRET` 或 `auth.jwt_secret`，这样登录 token 才能在服务重启后继续有效。
...
所有配置项均可通过环境变量覆盖，格式为 `LODESTAR_` + 配置路径（用 `_` 连接）：
...
| `LODESTAR_LOG_LEVEL` | `log.level` |
| `LODESTAR_AUTH_JWT_SECRET` | `auth.jwt_secret` |
| `LODESTAR_SECURITY_ENCRYPTION_KEY` | `security.encryption_key` |
| `LODESTAR_INITIAL_ADMIN_USERNAME` | 启动时自动创建初始管理员用户名 |
| `LODESTAR_INITIAL_ADMIN_PASSWORD` | 启动时自动创建初始管理员密码 |
| `LODESTAR_GITHUB_PAT` | 用于获取最新版本时的速率限制(可选) |
| `LODESTAR_RELAY_MAX_SSE_EVENT_SIZE` | 最大 SSE 事件大小(可选) |
...
以 `zen/<model>` 前缀发起的请求会绕过分组模型映射，直接路由到上游模型。Lodestar 会根据模型名进行智能渠道类型检测（如 Claude → Anthropic，Gemini → Gemini，GPT → OpenAI）。
...
这是最宽泛的视图 — 模型出现在这里，表示 Lodestar 有渠道*声明*了它。
...
> 部分端点之间的边界并非绝对。**对话族**（`chat`、`responses`、`messages`、`deepseek`、`mimo`）可以在 `endpoint` 过滤器中互相可见，因为 Lodestar 可以透明桥接这些格式。
...
- 当创建渠道或同步渠道模型时，如果某个模型还不在本地目录里，Lodestar 会自动创建本地价格记录，确保后续仍可手动维护价格
...
- 按可配置间隔自动同步渠道、令牌和模型
- 自动签到，支持可配置间隔和随机时间窗口
- **投射渠道**：从站点账号分组自动创建本地 Lodestar 渠道，支持按分组的 Key 管理、模型路由和历史追踪
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "sk-lodestar-your-key",
...
```json
{
  "OPENAI_API_KEY": "sk-lodestar-your-key"
}
```
