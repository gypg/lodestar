...
// Extended to support richer data from platforms like Sub2API (cache separation,
// model mapping, billing details). New fields are optional; existing adapters
// and sync logic remain compatible.
type RemoteUsageLog struct {
	...
	// Rich fields (populated by platforms that provide them)
	CacheReadTokens     int64  `json:"cache_read_tokens,omitempty"`
	CacheCreationTokens int64  `json:"cache_creation_tokens,omitempty"`
	RequestedModel      string `json:"requested_model,omitempty"` // model as requested by client
	UpstreamModel       string `json:"upstream_model,omitempty"`  // actual model used upstream
