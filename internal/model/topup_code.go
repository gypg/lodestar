...
type TopupCode struct {
	...
	Quota float64 `json:"quota" gorm:"type:double precision;not null"` // USD credited on redeem
	// Note carries the reconciliation trail for money collected outside the app:
	// who paid, how much, through which channel, when. Codes are the only inbound
	// money path when no payment provider is configured, and unlike PaymentOrder
	// they otherwise record nothing about the payment they represent — leaving the
	// operator to match a code against a bank transfer from memory months later.
	// Free-form on purpose: the shape of an offline receipt is not ours to dictate.
