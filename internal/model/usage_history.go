...
type RemoteUsageRecord struct {
	...
	// Extended rich fields (populated when the remote platform provides them).
	// These are optional to remain backward compatible with existing records.
	CacheReadTokens     int64  `json:"cache_read_tokens"`
	CacheCreationTokens int64  `json:"cache_creation_tokens"`
	RequestedModel      string `json:"requested_model" gorm:"size:255"`
	UpstreamModel       string `json:"upstream_model" gorm:"size:255"`
	BillingTier         string `json:"billing_tier" gorm:"size:64"`
	Group               string `json:"group" gorm:"size:64"`
	SubscriptionID      string `json:"subscription_id" gorm:"size:128"`
}
