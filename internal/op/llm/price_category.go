...
// 兜底匹配热点路径使用，避免每次查询都访问 DB。由 InitCache 与
// Create/Update/Delete PriceCategory 维护。
// atomic.Pointer：缓存刷新与热路径 PriceCategoryMatch 并发读写，
// 普通 slice 赋值存在数据竞争（slice 头非原子）。
...
// CreatePriceCategory 创建分类，name 转小写，rule_type/rule_value/价格校验。
...
func validatePriceCategory(c model.ModelPriceCategory) error {
	switch model.PriceCategoryRule(c.RuleType) {
	case model.PriceCategoryRuleExact,
		model.PriceCategoryRulePrefix,
		model.PriceCategoryRuleContains:
	default:
...
func categoryMatches(c model.ModelPriceCategory, modelName string) bool {
	rule := model.PriceCategoryRule(c.RuleType)
	...
	case model.PriceCategoryRuleExact:
		return modelName == v
	case model.PriceCategoryRulePrefix:
...
// ResetPriceCategoryCacheForTest 清空分类缓存快照，供测试隔离全局状态。
func ResetPriceCategoryCacheForTest() error {
