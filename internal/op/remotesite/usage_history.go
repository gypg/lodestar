...
func SyncUsageHistory(ctx context.Context, siteID int) (int, error) {
	...
	for page := 0; page < usageLogMaxPages; page++ {
		...
		// reachedCutoff marks that this page contains an already-synced or
		// out-of-window entry. Everything accumulated before that point is new
		// and must still be flushed, so the scan ends only after the insert.
		reachedCutoff := false
		for _, entry := range logs {
			...
			batch = append(batch, model.RemoteUsageRecord{
				...
				// Extended rich fields from remote platforms (e.g. Sub2API cache separation, model mapping, billing info)
