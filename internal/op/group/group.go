...
func GroupCreate(group *model.Group, ctx context.Context) error {
	...
	if err := condition.ValidateCondition(group.Condition); err != nil {
		return err
	}
...
func GroupUpdate(req *model.GroupUpdateRequest, ctx context.Context) (*model.Group, error) {
	...
	// Validate the condition before opening the transaction. The existing
	// GroupUpdate has a pre-existing bug: the "group name is required" early
	// return below skips tx.Rollback(), leaking a transaction. Validating here
	// (before Begin) avoids reproducing that leak for invalid-condition input.
	if req.Condition != nil {
		if err := condition.ValidateCondition(*req.Condition); err != nil {
			return nil, err
		}
	}
