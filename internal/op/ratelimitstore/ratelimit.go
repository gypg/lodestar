...
func rateLimitKey(apiKeyID int, modelName string) string {
	// model 段 TrimSpace+ToLower（octopus #238 + WO-045 阻断 4）：大小写与
	// 空白变体共享同一个限流桶，防止轮换变体绕过或碎片化 RPM/TPM 限额。
	return fmt.Sprintf("%d:%s", apiKeyID, strings.ToLower(strings.TrimSpace(modelName)))
}

// retryAfterSeconds converts a bucket's refill deadline into RFC 9110
// delta-seconds, rounding up so a partially elapsed second still asks the
// client to wait. Never returns less than 1 for a denied request.
func retryAfterSeconds(bucket *ratelimit.TokenBucket) int {
	d := time.Until(bucket.ResetAt())
	if d <= 0 {
		return 1
	}
...
func CheckRateLimit(apiKeyID int, modelName string, rpm int, tpm int, tokenCount int) (allowed bool, remaining int, retryAfter int) {
	...
	if rpm > 0 {
		...
		if !reqBucket.Allow() {
			return false, 0, retryAfterSeconds(reqBucket)
		}
	}
	...
	if tpm > 0 {
		...
		if !tokenBucket.AllowN(tokenCount) {
			return false, 0, retryAfterSeconds(tokenBucket)
		}
	}
