...
// buildConditionContext assembles the request metadata used by condition-based
// group routing. Every field the evaluator knows about must be populated here:
// an unset field is indistinguishable from a genuinely empty value, which turns
// a rule like `api_key_name not_equals X` into an always-true rule.
func buildConditionContext(c *gin.Context, requestModel string, apiKeyID int) condition.RequestContext {
	...
	if apiKey, err := ak.Get(apiKeyID, c.Request.Context()); err == nil {
		condCtx.APIKeyName = apiKey.Name
	}
	// resolveValue lowercases the part after "header_" before the lookup, so the
	// map must be keyed by lowercase names rather than Go's canonical form.
	headers := make(map[string]string, len(c.Request.Header))
	for name := range c.Request.Header {
		headers[strings.ToLower(name)] = c.GetHeader(name)
	}
	condCtx.Headers = headers
	return condCtx
}
