...
// settingKeyIs builds a cross-dialect WHERE condition on the "key" column,
// which is a reserved word in PostgreSQL/MySQL and must be quoted by the driver.
func settingKeyIs(key string) clause.Eq {
...
func cleanupNavSetting(db *gorm.DB, key string) error {
	...
	var setting Setting
	result := db.Where(settingKeyIs(key)).First(&setting)
	if result.Error != nil {
	...
	return db.Model(&Setting{}).Where(settingKeyIs(key)).Update("value", string(newJSON)).Error
}
...
func setNavDefault(db *gorm.DB, key string) error {
	...
	return db.Model(&Setting{}).Where(settingKeyIs(key)).Update("value", string(defaultJSON)).Error
}
