...
func dropLegacyChannelColumns(db *gorm.DB) error {
	...
	dropColumn := func(table, column string) error {
		// Quote identifiers per-dialect to avoid reserved-word syntax errors
		// (e.g. channels.key — "key" is reserved in PostgreSQL/MySQL).
		var tableRef, colRef string
		switch dialect {
		case "mysql":
			tableRef = "`" + table + "`"
			colRef = "`" + column + "`"
		case "postgres", "postgresql":
			tableRef = "\"" + table + "\""
			colRef = "\"" + column + "\""
		default: // sqlite and any unknown dialect
			tableRef = table
			colRef = column
		}
		var sql string
		switch dialect {
		case "postgres", "postgresql":
			sql = fmt.Sprintf("ALTER TABLE %s DROP COLUMN IF EXISTS %s", tableRef, colRef)
		default:
			sql = fmt.Sprintf("ALTER TABLE %s DROP COLUMN %s", tableRef, colRef)
		}
