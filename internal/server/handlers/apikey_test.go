...
func TestListAPIKeyMasksKeysForViewer(t *testing.T) {
	...
	apiKey := &model.APIKey{
		Name:    "viewer-key",
		APIKey:  "sk-lodestar-secret-12345678",
		Enabled: true,
	}
	...
	if got == "sk-lodestar-secret-12345678" {
		t.Fatalf("expected masked API key, got raw value %q", got)
	}
	if !strings.HasPrefix(got, "sk-l") || !strings.HasSuffix(got, "5678") {
...
func TestCreateAPIKeyIgnoresReadonlyFields(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/apikey/create", strings.NewReader(`{
		"id":1234,
		"name":"created-key",
		"api_key":"sk-lodestar-client-supplied",
		"enabled":true
	}`))
	...
	if response.Data.APIKey != "sk-lodestar-client-supplied" {
...
func TestCreateAPIKeyAutoGeneratesWhenEmpty(t *testing.T) {
	...
	if !strings.HasPrefix(response.Data.APIKey, "sk-lodestar-") {
		t.Fatalf("expected auto-generated key with sk-lodestar- prefix, got %q", response.Data.APIKey)
	}
}
...
func TestCreateAPIKeyAcceptsFullyCustomValue(t *testing.T) {
	...
	// The backend should auto-prepend the sk-lodestar- prefix.
	if response.Data.APIKey != "sk-lodestar-my-totally-custom-api-key" {
...
func TestUpdateAPIKeyDoesNotEchoRawKey(t *testing.T) {
	...
	apiKey := &model.APIKey{
		Name:    "updatable-key",
		APIKey:  "sk-lodestar-secret-abcdef12",
		Enabled: true,
	}
	if err := op.APIKeyCreate(apiKey, ctx); err != nil {
		t.Fatalf("create api key: %v", err)
	...
	if response.Data.APIKey == "sk-lodestar-secret-abcdef12" {
