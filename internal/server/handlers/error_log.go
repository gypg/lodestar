...
// reportRateLimitPerMinute 限制单个登录用户的上报频率（每分钟条数），防止
...
// resetReportRateLimitsForTest 清空上报限流表，供测试隔离全局状态。
func resetReportRateLimitsForTest() {
	reportRateLimits.Lock()
	defer reportRateLimits.Unlock()
	reportRateLimits.items = make(map[string]struct {
		count int
		at    time.Time
	})
}
...
func reportErrorLog(c *gin.Context) {
	...
	message = xstrings.TruncateRunes(message, 8192)
	stack := xstrings.TruncateRunes(req.Stack, 65536)
	...
	entry := model.ErrorLog{
		...
		Message:   message,
		Stack:     stack,
		PageURL:   xstrings.TruncateRunes(req.PageURL, 1024),
		RouteID:   xstrings.TruncateRunes(req.RouteID, 128),
		UserAgent: xstrings.TruncateRunes(c.Request.UserAgent(), 512),
		ClientIP:  c.ClientIP(),
		Version:   xstrings.TruncateRunes(req.Version, 64),
	}
